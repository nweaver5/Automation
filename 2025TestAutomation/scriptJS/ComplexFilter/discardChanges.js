...
async function changeDiscardFilter(driver){
  ...
  let discardChanges = await driver.executeScript(
    COMPLEX_FILTER_SELECTORS.CUSTOM_FILTER_DISCARD_CHANGES_SELECTOR
  );
  await driver.actions().click(discardChanges).perform();
  await universalHelpers.sleep(1000);

  let operationInputValue = await driver.executeScript(
    COMPLEX_FILTER_SELECTORS.OPERATION_INPUT_SELECTOR
  );
  let optionInputValue = await driver.executeScript(
    COMPLEX_FILTER_SELECTORS.OPTION_INPUT_SELECTOR
  );

  assert(operationInputValue == "is");
  assert(optionInputValue == "Open");
...
export default async function discardChanges(driver) {
  try {
    // Navigating to Tasks
    ...
    console.log("Change, Discard Filter successful.")
  } catch (error) {
    console.error(error, "Change, Discard Filter failed.");
    await universalHelpers.takeScreenshot(driver);
    await driver.quit();
  }
}
