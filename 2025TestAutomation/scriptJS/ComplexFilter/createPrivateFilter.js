...
async function saveFilter(driver) {
  ...
  await driver
    .actions()
    .sendKeys(filterNameInput, "Automation Filter Private")
    .perform();
...
async function verifyFilter(driver) {
  try {
    ...
    assert(filterHelpers.checkForFilter(privateFilterListItems, "Automation Filter Private"), "Filter not found");
  } catch (error) {
...
export default async function createPrivateFilter(driver) {
  try {
    ...
    console.log("Create Private Filter successful.");
  } catch (error) {
    await universalHelpers.catchError(driver, error, "Create Filter Failed");
  }
}
